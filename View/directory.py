...
from Controller import dirController


def dir_decorator(func):
    def wrapper(self, *args, **kwargs):
        print("Changing to Directory Page")
        func(self, *args, **kwargs)
        self.show()
    return wrapper

class DirPage(QWidget):
    def __init__(self):
        super().__init__()
        self.init_ui()
    ...
    def init_ui(self) -> None:
        ...
        dirController.manage_dir_list(list)

        buttonLayout = QVBoxLayout()
        addButton = QPushButton("+")
        addButton.clicked.connect(lambda: dirController.button_clicked(self, "+", list))
        buttonLayout.addWidget(addButton)

        removeButton = QPushButton("-")
        removeButton.clicked.connect(lambda: dirController.button_clicked(self, "-", list))
