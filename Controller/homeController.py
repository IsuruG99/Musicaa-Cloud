...
def playMusic(action: str) -> None:
    ...
    def play_next(index: int) -> None:
        ...
        try:
            ...
            # Set the event to play the next song when the current one ends
            pygame.mixer.music.set_endevent(pygame.USEREVENT)
            pygame.event.set_blocked(pygame.USEREVENT)
            pygame.event.post(pygame.event.Event(pygame.USEREVENT, {"index": index + 1}))
    ...
    if action == "play":
        ...
        play_next(current_index + 1)
    elif action == "prev":
        play_next(current_index - 1 if current_index > 0 else len(playlist) - 1)
